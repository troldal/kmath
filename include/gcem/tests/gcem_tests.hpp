...
inline void print_test_pass(std::string fn_name, const int print_level,
                            int print_precision_1, int print_precision_2,
                            const T1 f_val, const T2 err_val, const T3 par_1) {
  std::cout << "[\033[32mOK\033[0m] ";
  std::cout << std::setiosflags(std::ios::fixed)
            << std::setprecision(print_precision_1) << fn_name << "(" << par_1
...
inline void print_test_pass(std::string fn_name, const int print_level,
  ...
  std::cout << std::setiosflags(std::ios::fixed)
            << std::setprecision(print_precision_1) << fn_name << "(" << par_1
            << "," << par_2 << ") = " << std::setprecision(print_precision_2)
            << f_val << "\n";
...
template <typename T1, typename T2, typename T3>
inline void print_test_fail(std::string fn_name, int test_number,
                            const int print_level, int print_precision_1,
                            int print_precision_2, const T1 f_val,
                            const T2 expected_val, const T3 par_1) {
...
print_test_fail(std::string fn_name, int test_number, const int print_level,
                int print_precision_1, int print_precision_2, const T1 f_val,
                const T2 expected_val, const T3 par_1, const T4 par_2) {
  GCEM_UNUSED_PAR(print_level);
  GCEM_UNUSED_PAR(print_precision_1);
  GCEM_UNUSED_PAR(print_precision_2);
...
#define GCEM_TEST_EXPECTED_VAL(gcem_fn, expected_val, ...)                     \
  {                                                                            \
    ...
    auto f_val = gcem_fn(__VA_ARGS__);                                         \
                                                                               \
    auto err_val =                                                             \
        std::abs(f_val - expected_val) / (1 + std::abs(expected_val));         \
                                                                               \
    bool test_success = false;                                                 \
                                                                               \
    if (VAL_IS_NAN(expected_val) && VAL_IS_NAN(f_val)) {                       \
      test_success = true;                                                     \
    } else if (!VAL_IS_NAN(f_val) && VAL_IS_INF(f_val) &&                      \
               f_val == expected_val) {                                        \
      test_success = true;                                                     \
      ...
      print_test_fail(fn_name, GCEM_TEST_NUMBER, TEST_PRINT_LEVEL,             \
                      TEST_PRINT_PRECISION_1, TEST_PRINT_PRECISION_2, f_val,   \
                      expected_val, __VA_ARGS__);                              \
    }                                                                          \
                                                                               \
    if (test_success && TEST_PRINT_LEVEL > 0) {                                \
      print_test_pass(fn_name, TEST_PRINT_LEVEL, TEST_PRINT_PRECISION_1,       \
                      TEST_PRINT_PRECISION_2, f_val, err_val, __VA_ARGS__);    \
    }                                                                          \
  }
...
#define GCEM_TEST_COMPARE_VALS(gcem_fn, std_fn, ...)                           \
  {                                                                            \
    ...
    auto std_fn_val = std_fn(__VA_ARGS__);                                     \
                                                                               \
    auto err_val =                                                             \
        std::abs(gcem_fn_val - std_fn_val) / (1 + std::abs(std_fn_val));       \
                                                                               \
    bool test_success = false;                                                 \
                                                                               \
    if (VAL_IS_NAN(gcem_fn_val) && VAL_IS_NAN(std_fn_val)) {                   \
      test_success = true;                                                     \
    } else if (!VAL_IS_NAN(gcem_fn_val) && VAL_IS_INF(gcem_fn_val) &&          \
               gcem_fn_val == std_fn_val) {                                    \
      test_success = true;                                                     \
    } else if (err_val < TEST_ERR_TOL) {                                       \
      test_success = true;                                                     \
    } else {                                                                   \
    ...
    if (test_success && TEST_PRINT_LEVEL > 0) {                                \
      print_test_pass(fn_name, TEST_PRINT_LEVEL, TEST_PRINT_PRECISION_1,       \
                      TEST_PRINT_PRECISION_2, gcem_fn_val, err_val,            \
                      __VA_ARGS__);                                            \
    }                                                                          \
  }
...
inline void finish_print_line(int k) {
...
inline void print_begin(std::string fn_name) {
...
inline void print_final(std::string fn_name) {
  ...
  std::cout << "#  - Test suite: " << fn_name;
  finish_print_line(14 + int(fn_name.length()) + 4);
  std::cout << std::endl;

  //

  std::cout << "#  - Number of tests: " << GCEM_TEST_NUMBER;
  finish_print_line(20 + (GCEM_TEST_NUMBER > 9 ? 1 : 0) + 4);
  std::cout << std::endl;

  //

  std::cout << "#  - Error tolerance: " << std::scientific << TEST_ERR_TOL;
