...
template <typename T> constexpr T lgamma_term_2(const T x) noexcept { //
  return (T(GCEM_LOG_SQRT_2PI) + log(T(lgamma_coef_term(x))));
}

template <typename T>
constexpr T
lgamma_term_1(const T x) noexcept { // note: 607/128 + 0.5 = 5.2421875
...
template <typename T> constexpr T lgamma_check(const T x) noexcept {
  return ( // NaN check
