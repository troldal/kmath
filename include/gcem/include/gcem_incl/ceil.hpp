...
template <typename T> constexpr T ceil_check_internal(const T x) noexcept {
  return x;
}

template <> constexpr float ceil_check_internal<float>(const float x) noexcept {
...
ceil_check_internal<long double>(const long double x) noexcept {
  return (
      abs(x) >= 9223372036854775808.l ? // if
          x
                                      : // else
          ceil_int(x, ((long double)static_cast<ullint_t>(abs(x))) * sgn(x)));
}

template <typename T> constexpr T ceil_check(const T x) noexcept {
  return ( // NaN check
