...
template <typename T> constexpr bool is_neginf(const T x) noexcept {
...
template <typename T> constexpr bool is_posinf(const T x) noexcept {
  return x == GCLIM<T>::infinity();
}
...
template <typename T> constexpr bool is_inf(const T x) noexcept {
