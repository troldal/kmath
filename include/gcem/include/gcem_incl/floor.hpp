...
template <typename T> constexpr T floor_check_internal(const T x) noexcept {
  return x;
}
...
floor_check_internal<long double>(const long double x) noexcept {
  return (
...
template <typename T> constexpr T floor_check(const T x) noexcept {
  return ( // NaN check
