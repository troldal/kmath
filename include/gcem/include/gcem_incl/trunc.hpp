...
template <typename T> constexpr T trunc_int(const T x) noexcept {
  return (T(static_cast<llint_t>(x)));
}

template <typename T> constexpr T trunc_check_internal(const T x) noexcept {
  return x;
}
...
trunc_check_internal<long double>(const long double x) noexcept {
...
template <typename T> constexpr T trunc_check(const T x) noexcept {
  return ( // NaN check
