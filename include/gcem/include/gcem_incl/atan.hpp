...
template <typename T>
constexpr T atan_series_order_calc(const T xx, const T x_pow,
                                   const uint_t order) noexcept {
...
template <typename T>
constexpr T atan_series_order(const T x, const T x_pow,
                              const uint_t order_begin,
...
constexpr T atan_series_order(const T x, const T x_pow, const uint_t order,
                              const uint_t max_order) noexcept {
  return (max_order == 1 ? GCEM_HALF_PI - T(1) / x
          : order == 1
              ? GCEM_HALF_PI - T(1) / x +
...
template <typename T> constexpr T atan_series_main(const T x) noexcept {
...
template <typename T>
constexpr T atan_cf_recur(const T xx, const uint_t depth_begin,
                          const uint_t max_depth) noexcept {
...
template <typename T>
constexpr T atan_cf_recur(const T xx, const uint_t depth,
                          const uint_t max_depth) noexcept {
  return (depth < max_depth ? // if
              T(2 * depth - 1) + T(depth * depth) * xx /
...
template <typename T> constexpr T atan_cf_main(const T x) noexcept {
...
template <typename T> constexpr T atan_begin(const T x) noexcept {
  return (x > T(2.5) ? atan_series_main(x) : atan_cf_main(x));
}

// check input

template <typename T> constexpr T atan_check(const T x) noexcept {
  return ( // NaN check
