...
template <typename T> constexpr T log_cf_begin(const T x) noexcept {
  return (T(2) * x / log_cf_main(x * x, 1));
}

template <typename T> constexpr T log_main(const T x) noexcept {
...
template <typename T> constexpr T log_check(const T x) noexcept {
...
constexpr return_t<T> log_integral_check(const T x) noexcept {
  return (std::is_integral<T>::value
              ? x == T(0)  ? -GCLIM<return_t<T>>::infinity()
