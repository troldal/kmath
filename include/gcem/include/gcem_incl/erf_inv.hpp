...
template <typename T>
constexpr T erf_inv_decision(const T value, const T p, const T direc,
...
template <typename T>
constexpr T erf_inv_initial_val_coef_2(const T a, const T p_term,
...
template <typename T>
constexpr T erf_inv_initial_val_case_2(const T a, const T p_term,
                                       const int order) noexcept {
  return (order == 9 ? // if
...
template <typename T>
constexpr T erf_inv_initial_val_coef_1(const T a, const T p_term,
                                       const int order) noexcept {
...
template <typename T>
constexpr T erf_inv_initial_val_case_1(const T a, const T p_term,
                                       const int order) noexcept {
  return (order == 9 ? // if
...
template <typename T> constexpr T erf_inv_initial_val_int(const T a) noexcept {
  return (a < T(5) ? // if
...
template <typename T> constexpr T erf_inv_initial_val(const T x) noexcept {
...
template <typename T>
constexpr T erf_inv_err_val(const T value,
...
template <typename T>
constexpr T
erf_inv_deriv_2(const T value,
                const T deriv_1) noexcept { // second derivative of the error
                                            // function w.r.t. x
  return (deriv_1 * (-T(2) * value));
}

template <typename T>
constexpr T erf_inv_ratio_val_1(const T value, const T p,
...
template <typename T>
constexpr T erf_inv_recur(const T value, const T p, const T deriv_1,
                          const int iter_count) noexcept {
...
constexpr T erf_inv_decision(const T value, const T p, const T direc,
  ...
  return (iter_count < GCEM_ERF_INV_MAX_ITER ? // if
              erf_inv_recur(value - direc, p, erf_inv_deriv_1(value),
                            iter_count + 1)
                                             :
                                             // else
              value - direc);
...
template <typename T> constexpr T erf_inv_begin(const T p) noexcept {
  return ( // NaN check
